  ...
  return (
    <div className="min-h-screen bg-surface-off flex flex-col items-center py-8 px-4 font-body">
      <div className="w-full max-w-lg">
          ...
          {phase === 'playing' && (
            <motion.div key={`g${gIdx}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex gap-3 mb-3">
                <button onClick={() => { window.parent.postMessage({ type: 'HINT_USED' }, '*'); setMsg(g.hint); }}
        ...
        <p className="mt-8 text-text-muted text-xs font-mono text-center">ESUG Interactive Games — Graph Coloring</p>
      </div>
    </div>
  );
}
