...
const PUZZLES = [
  {
    ...
    // Col 1: 3 − 1 = 2 ✓
    // Col 2: 4 + 3 = 7 ✓
    solution: [7, 3, 4,  2, 1, 3,  9, 2, 7],
    rowOps: ['-', '+', '-'],
    colOps: ['+', '-', '+'],
    clues: [0, 5, 8],   // pre-filled: s[0]=7, s[5]=3, s[8]=7
    // bank = only the 6 blank values, sorted
    bank: [1, 2, 2, 3, 4, 9],
  },
  {
    // Row 0: 12 − 4 = 8 ✓
    ...
    // Col 1:  4 − 1 = 3 ✓
    // Col 2:  8 − 6 = 2 ✓
    solution: [12, 4, 8,  7, 1, 6,  5, 3, 2],
    rowOps: ['-', '-', '-'],
    colOps: ['-', '-', '-'],
    clues: [2, 4, 6],   // pre-filled: s[2]=8, s[4]=1, s[6]=5
    bank: [2, 3, 4, 6, 7, 12],
  },
  {
    // Row 0:  1 + 8 =  9 ✓
    ...
    // Col 1:  8 + 3 = 11 ✓
    // Col 2:  9 + 7 = 16 ✓
    solution: [1, 8, 9,  4, 3, 7,  5, 11, 16],
    rowOps: ['+', '+', '+'],
    colOps: ['+', '+', '+'],
    clues: [1, 3, 8],   // pre-filled: s[1]=8, s[3]=4, s[8]=16
    bank: [1, 3, 5, 7, 9, 11],
  },
];
...
function ValCell({ value, isClue, isSelected, rowResult, colResult, onClick }) {
  ...
  return (
    <motion.button onClick={onClick} whileTap={!isClue ? {scale:0.92} : {}}
      disabled={isClue}
      className="w-12 h-12 rounded-xl border-2 flex items-center justify-center font-mono font-black text-lg transition-all select-none"
...
export default function MathCross() {
  ...
  const [selBank,  setSelBank]= useState(null); // bank tile index selected
  ...
  // Compute per-cell feedback (only after Check is pressed)
  ...
  function clickCell(si) {
    ...
    const newGrid = [...grid];
    if (newGrid[si] !== null) {
      // clicking an already-filled cell: clear it
      newGrid[si] = null;
  ...
  function clickFilled(si) {
    // Right-click or second click on filled cell removes it
    if (puzzle.clues.includes(si)) return;
    const newGrid = [...grid];
    newGrid[si] = null;
    setGrid(newGrid);
  ...
  function submit() {
    ...
    if (allOk) {
      ...
      setTimeout(() => {
        ...
        else { setPIdx(i => i+1); setGrid(initGrid(pIdx+1)); setSelBank(null); setChecked(false); setMsg(''); }
      }, 1000);
    } else {
      const t = tries - 1; setTries(t);
      if (t <= 0) setPhase('lost');
      else setMsg(`Some equations don't balance — wrong cells highlighted. ${t} tries left.`);
    }
  }
  ...
  function giveHint() {
    ...
    if (empty === -1) return;
    const ng = [...grid]; ng[empty] = puzzle.solution[empty];
    setGrid(ng); setHints(h => h+1); setChecked(false);
    setMsg(`Hint: one cell filled (−25 XP)`);
  }

  function reset() {
    setPIdx(0); setGrid(initGrid(0)); setSelBank(null);
    setTries(MAX_TRIES); setScore(0); setPhase('playing');
    setChecked(false); setMsg(''); setHints(0);
  }

  // Which bank tiles are placed in the grid?
  const usedBankIdx = new Set();
  grid.forEach(v => {
    if (v !== null && !puzzle.clues.includes(grid.indexOf(v))) {
      // find an un-used bank slot with this value
      puzzle.bank.forEach((bv, bi) => {
        if (bv === v && !usedBankIdx.has(bi)) { usedBankIdx.add(bi); }
      });
    }
  });

  // Better used tracking: iterate slots
  const usedBank = (() => {
    const used = new Set();
    grid.forEach((v, si) => {
    ...
    <div className="min-h-screen bg-surface-off flex flex-col items-center py-8 px-4 font-body">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-text-mid text-sm mb-5">
          Place numbers so every row AND column equation balances.
        </p>
          ...
          {phase === 'intro' && (
            <motion.div key="intro" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}}
              ...
              <div className="space-y-3 mb-5">
                {[
                  'A 3×3 grid of number cells is shown. Some are pre-filled (grey). Six cells are blank.',
                  'Select a number from the bank below, then click a blank cell to place it.',
                  'Every <strong>row</strong> equation must balance (left op middle = right).',
                  'Every <strong>column</strong> equation must also balance (top op middle = bottom).',
                  'Click a filled cell to remove it if you change your mind.',
                ].map((s,i) => (
              ...
              <div className="bg-[#1CB0F6]/8 rounded-2xl p-4 mb-5 border border-[#1CB0F6]/20">
                ...
                <p className="font-body text-sm text-text-mid">Rows read left → right. Columns read top → bottom. The <strong>= signs</strong> mark the end of each equation.</p>
              </div>
                  ...
                  {puzzle.bank.map((n, bi) => {
                    ...
                    return (
                      <motion.button key={bi}
                        ...
                        className={['w-12 h-12 rounded-xl border-2 font-mono font-black text-base transition-all',
